        ...
        <div class="box-content clearfix">
            ...
            <ul class="nav nav-pills">
                @foreach (supported_locales() as $locale => $language)
                    <li class="nav-item">
                        <a href="#storeNameTabs{{ $locale }}" data-bs-toggle="tab" aria-expanded="true" class="nav-link {{ $locale === locale() ? 'active' : '' }}">
            ...
            <div class="tab-content pt-2 text-muted">
                @foreach (supported_locales() as $locale => $language)
                    <div class="tab-pane {{ $locale === locale() ? 'show active' : '' }}" id="storeNameTabs{{ $locale }}">
                        {{ Form::text('translatable[store_name][' . $locale . ']', trans('setting::attributes.translatable.store_name'), $errors, $settings) }}
                    </div>
                @endforeach
            </div>
            ...
            {{ Form::text('store_phone2', trans('setting::attributes.store_phone'), $errors, $settings, ['required' => false]) }}
            {{ Form::text('store_phone3', trans('setting::attributes.store_phone'), $errors, $settings, ['required' => false]) }}
