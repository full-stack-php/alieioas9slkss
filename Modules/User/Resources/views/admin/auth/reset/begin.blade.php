                    ...
                    <div class="row justify-content-center h-100">
                        <div class="col-lg-6 py-lg-5">
                            <div class="d-flex flex-column h-100 justify-content-center">
                                ...
                                <h2 class="fw-bold fs-24">{{ trans('user::auth.reset_password') }}</h2>

                                <p class="text-muted mt-1 mb-4">{{ trans('user::auth.enter_email') }}</p>

                                <div>
                                    <form class="authentication-form" method="POST" action="{{ route('admin.reset.post') }}">
                                        {{ csrf_field() }}
                                        <div class="mb-3">
                                            <label class="form-label" for="email"> {{ trans('user::auth.email') }}</label>
                                            <input type="text" id="email" name="email" class="form-control" placeholder="{{ trans('user::auth.enter_your_email') }}">
