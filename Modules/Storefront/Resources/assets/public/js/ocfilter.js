...
!function (a, b) {
    ...
    !function () {
        a.register("locale", "bg", {
            delimiters: {thousands: " ", decimal: ","},
            abbreviations: {thousand: "Ñ…Ð¸Ð»", million: "Ð¼Ð»Ð½", billion: "Ð¼Ð»Ñ€Ð´", trillion: "Ñ‚Ñ€Ð»Ð½"},
            ordinal: function (a) {
                return ""
            },
            currency: {symbol: "Ð»Ð²"}
        ...
        a.register("locale", "ru", {
            delimiters: {thousands: " ", decimal: ","},
            abbreviations: {thousand: "Ñ‚Ñ‹Ñ.", million: "Ð¼Ð»Ð½.", billion: "Ð¼Ð»Ñ€Ð´.", trillion: "Ñ‚Ñ€Ð»Ð½."},
            ordinal: function () {
                return "."
            },
            currency: {symbol: "Ñ€ÑƒÐ±."}
        })
    }(), function () {
        ...
        a.register("locale", "vi", {
            delimiters: {thousands: ".", decimal: ","},
            abbreviations: {thousand: " nghÃ¬n", million: " triá»‡u", billion: " tá»·", trillion: " nghÃ¬n tá»·"},
            ordinal: function () {
                return "."
            },
            currency: {symbol: "â‚«"}
...
+function (t) {
    ...
    function a(e) {
        return this.each(function () {
            var $this = t(this); // Используем $this для ясности
            var data = $this.data(OCF_PREFIX + ".collapse");

            // В jQuery 4 важно четко проверять тип e
            var options = t.extend({}, i.DEFAULTS, $this.data(), typeof e === 'object' && e);

            // 1. Исправляем проверку регулярным выражением
            // Если e — это объект события, String(e) вернет "[object Object]", что завалит .test()
            var command = typeof e === 'string' ? e : null;

            if (!data && options.toggle && /show|hide/.test(command)) {
                options.toggle = false;
            }

            // 2. Инициализация объекта, если его еще нет
            if (!data) {
                data = new i(this, options);
                $this.data(OCF_PREFIX + ".collapse", data);
            }

            // 3. Безопасный вызов метода
            if (command && typeof data[command] === 'function') {
                data[command]();
            }
        });
    }
        ...
        if (!this.transitioning && !this.$element.hasClass(OCF_PREFIX + "-in")) {
            ...
            if (!(s && s.length && (e = s.data(OCF_PREFIX + ".collapse"), e && e.transitioning))) {
                ...
                if (this.$element.trigger(n), !n.isDefaultPrevented()) {
                    ...
                    var r = ["scroll", l].join("-").replace(/-([a-z])/g, function(g) {
...
+function ($) {
    ...
    OCFilter.LINK_SEARCH = '#';
    OCFilter.LINK_FILTERS = '#';
    OCFilter.LINK_VALUES = '#';
    ...
    OCFilter.prototype.init = function () {
        ...
        this.$element.on('click.' + OCF_PREFIX, function (e) {
            ...
            var isRadio = $value.hasClass(OCF_PREFIX + '-radio'),
                $filterItem = $value.closest('.' + that.filterItemClass), filter_key = $value.attr('data-filter-key');
        ...
        function loadData(e) {
            ...
            var $collapse = $(this),
                $button = $('[data-target="#' + $collapse.attr('id') + '"]')[that.button]('loading'),
