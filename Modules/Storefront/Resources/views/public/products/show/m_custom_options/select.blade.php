...
<div class="form-group required">
    ...
    <select class="form-control second_option"
            id="m_option-{{ $option->id }}"
            style="display: none;"
        {{ $isSingleValue ? 'disabled' : "name=m_options[{$option->id}]" }}>
    ...
    @if($isSingleValue)
        ...
        <input type="hidden" name="m_options[{{ $option->id }}]" value="{{ $singleValue->id }}">
    @endif
</div>
