@hasAccess('admin.products.index')
    ...
    @if (auth()->user()->hasAccess('admin.categories.index') && ! ($featuredCategories ?? false))
        <div class="category-products {{ setting("{$fieldNamePrefix}_product_type") === 'category_products' ? '' : 'd-none' }}">
            {{ Form::select("{$fieldNamePrefix}_category_id", trans('storefront::attributes.category'), $errors, $categories, $settings) }}
        </div>
    @endif

    <div class="products-limit {{ in_array(setting("{$fieldNamePrefix}_product_type"), ['latest_products', 'recently_viewed_products','category_products']) ? '' : 'd-none' }}">
        {{ Form::number("{$fieldNamePrefix}_products_limit", trans('storefront::attributes.products_limit'), $errors, $settings) }}
    </div>

    <div class="custom-products {{ setting("{$fieldNamePrefix}_product_type") === 'custom_products' ? '' : 'd-none' }}">
