...
const globalOptions = ref(Korf.data["global-options"] ?? []);
...
function changeOptionType(index, uid) {
    ...
    const fieldName =
        isOptionTypeText(option) || isOptionTypeDate(option)
            ? "price"
            : "label";
...
function isOptionTypeDate(option) {
    const TYPES = ["date", "date_time", "time"];
