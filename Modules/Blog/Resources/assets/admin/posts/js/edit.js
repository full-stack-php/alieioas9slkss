...
Alpine.data("postEdit", ({ formData = {}, meta = {}, tags = [] }) => ({
    ...
    init() {
        ...
        this.fillTranslationFields(this.form);
    ...
    fillTranslationFields(data) {
        if (!data.translations_data) return;

        data.translations_data.forEach((item) => {
            const locale = item.locale;

            // Заполнение поля 'name'
            const nameSelector = `[name="${locale}[name]"]`;
            $(nameSelector).val(item.name || '');

            // Заполнение поля 'description' (если оно не управляется TinyMCE)
            const descriptionSelector = `[name="${locale}[description]"]`;
            $(descriptionSelector).val(item.description || '');

            // Если TinyMCE уже инициализирован, установите его контент:
            if (textEditor) {
                textEditor.get(descriptionSelector).setContent(item.description || '');
            }

            // Если у вас есть другие переводимые поля (h1_name, slug), добавьте их здесь:
            // $(`[name="${locale}[h1_name]"]`).val(item.h1_name || '');
        });

        // Заполнение полей мета-тегов
        if (data.meta_data) {
            data.meta_data.forEach((item) => {
                const locale = item.locale;
                $(`[name="meta[${locale}][meta_title]"]`).val(item.meta_title || '');
                $(`[name="meta[${locale}[meta_description]"]`).val(item.meta_description || '');
            });
        }
    },
    initTinyMce() {
