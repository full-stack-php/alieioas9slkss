...
import initTranslationFields from "@admin/js/translate";
...
window.trans = function trans(langKey, replace = {}) {
    let line = window.Korf.langs[langKey];
...
class FormAdvanced {
    ...
    initFormChoices() {
        ...
        choicesExamples.forEach(function (item) {
            ...
            const choices = isChoicesVal["data-choices-text-disabled-true"] ? new Choices(item, choiceData).disable() : new Choices(item, choiceData);

            if (item.dataset.url) {
                const url = item.dataset.url;

                item.addEventListener('search', _.debounce(function(event) {
