...
export default class {
    ...
    confirmationModal() {
        const confirmationModalElement = document.getElementById("confirmation-modal"); // 1. Получаем DOM-элемент

        // 2. Создаем или получаем экземпляр Modal
        let confirmationModalInstance = bootstrap.Modal.getInstance(confirmationModalElement);
        if (!confirmationModalInstance) {
            confirmationModalInstance = new bootstrap.Modal(confirmationModalElement);
        }

        // jQuery-объект для удобства поиска элементов внутри модального окна
        ...
        confirmationModalElement.addEventListener("shown.bs.modal", () => {
